...
<script>
  import { fetchDetail, createPaper } from '@/api/exam'
  ...
  export default {
    ...
    methods: {
      ...
      handleCreate() {
        ...
        this.showLoading()
        ctx.$router.push({ name: 'startExam', params: { id: '1305039775765442562' }})
        ...
        createPaper(params).then(response => {
          ...
          if (response.code === 0) {
            ...
            setTimeout(function() {
              ctx.$router.push({ name: '开始考试', params: { id: response.data.id }})
            }, 2000)
          } else {
